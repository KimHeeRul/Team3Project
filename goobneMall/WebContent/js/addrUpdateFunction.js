function addrUpdateformChk() {
	const form = $("#addrUpdateForm");
	...
	const PhoneFst = $("select[name=addrUpdate_PhoneFst]").val();
	const PhoneScd = $("input[name=addrUpdate_PhoneScd]").val();
	const PhoneTrd = $("input[name=addrUpdate_PhoneTrd]").val();

	const HPhoneFst = $("select[name=addrUpdate_HPhoneFst]").val();
	const HPhoneScd = $("input[name=addrUpdate_HPhoneScd]").val();
	const HPhoneTrd = $("input[name=addrUpdate_HPhoneTrd]").val();
	...
	if(addrName === ""){
		...
		$("input[name=addrUpdate_PhoneScd]").focus();
	}
	else if(!reg2.test(PhoneTrd)){
		alert(" 핸드폰 번호를 확인해주세요");
		$("input[name=addrUpdate_PhoneTrd]").focus();
	}
	else if(HPhoneFst != "선택"){ // 지역번호 입력 되었을 때
		if(!reg1.test(HPhoneScd)){
			alert(" 전화번호를 확인해주세요");
			$("input[name=addrUpdate_HPhoneScd]").focus();
		}
		else if (!reg1.test(HPhoneTrd)){
			alert(" 전화번호를 확인해주세요");
			$("input[name=addrUpdate_HPhoneTrd]").focus();
		}
		else {
			form.submit();
		}
		...
		if(HPhoneScd != "" || HPhoneTrd != ""){
			alert("지역번호를 선택해주세요");
			$("select[name=addrUpdate_HPhoneFst]").focus();
		}
