...
<script type="text/javascript" src="/goobneMall/js/addrUpdateFunction.js?v=<%=System.currentTimeMillis() %>"></script>
<style>
.addr_table td.info {
	width: 100px;
}
...
<div id="snb">
	...
	<div id="my_address">
		<h3 style="margin-bottom: 15px;">배송지 수정</h3>
		...
		<div>
			...
			<div style="display: flex; align-items:center; height:30px;  justify-content: space-between;">
				...
				<form action="/goobneMall/service" id="addrUpdateForm" method="post">
					<input type="hidden" name="command" value="addrUpdate">
					<input type="hidden" name="addr_no" value='<c:out value="${requestScope.addrUpdateObject.getAddr_no()}"/>'>
					<table class="addr_table">
						<tr>
							...
							<input type="text" id="addrname" name="addrname" class="form-control"
								style="width: 45%;" maxlength="20" placeholder="배송지명" value='<c:out value="${requestScope.addrUpdateObject.getAddrname()}"/>' ></td>
						</tr>
						<tr>
							<td class="info">
								<div style="display: flex; text-align:left; align-items: center;">
							...
							<td class="addr_input_content"><input type="text"
								...
								class="form-control" placeholder="이름" value='<c:out value="${requestScope.addrUpdateObject.getName()}"/>'></td>
						</tr>
						<tr>
							<td class="info">
								<div style="display: flex; text-align:left; align-items: center;">
							...
							<td class="addr_input_content">
								<div
									...
									<input type="text" class="form-control"style="display: inline-block; width: 30%; margin-right: 15px;"
										id="sample6_postcode" name="input_addrNum" placeholder="우편번호" value='<c:out value="${requestScope.addrUpdateObject.getAddrNum()}"/>'> 
								...
								<div
									...
									<input type="text" class="form-control"
										...
										placeholder="주소" name="input_nomalAddr" value='<c:out value="${requestScope.addrUpdateObject.getAddr()}"/>'>
									<input type="text"
										class="form-control" style="width: 60%; margin-right: 10px;"
										id="sample6_detailAddress" name="input_detailAddr" placeholder="상세주소" value='<c:out value="${requestScope.addrUpdateObject.getDetailAddr()}"/>'> 
									<!-- <input
						...
						<tr>
							...
							<select id="addrUpdateSelPhone"
								name="addrUpdate_PhoneFst"
							...
							<input type="text" id="phone"
								name="addrUpdate_PhoneScd" class="form-control" style="width: 15%"
								maxlength="4"> 
								<span>-</span> 
							<input type="text"
								id="phone" name="addrUpdate_PhoneTrd" class="form-control"
								style="width: 15%" maxlength="4">
								
								
							<script type="text/javascript">
								const option = '<c:out value="${requestScope.addrUpdateObject.getPhone()}" />';
								const arr = option.split("-");
								$('#addrUpdateSelPhone').val(arr[0]).prop("selected",true);
								$('input[name=addrUpdate_PhoneScd]').val(arr[1]);
								$('input[name=addrUpdate_PhoneTrd]').val(arr[2]);
							</script>
							</td>
						</tr>
						<tr>
							...
							<td style="text-align: left;">
							<select id="addrUpdateHPhone"
								name="addrUpdate_HPhoneFst"
							...
							<span>-</span> 
							<input type="text" id="phone"
								name="addrUpdate_HPhoneScd" class="form-control" style="width: 15%"
								maxlength="4"> 
								<span>-</span> 
							<input type="text"
								id="phone" name="addrUpdate_HPhoneTrd" class="form-control"
								style="width: 15%" maxlength="4">
							<script type="text/javascript">
								const option2 = '<c:out value="${requestScope.addrUpdateObject.getHphone()}" />';
								// console.log(option2);
								// console.log(option2===""); // true / null : false
								if(option2 != ""){
									const arr = option2.split("-");
									$('#addrUpdateHPhone').val(arr[0]).prop("selected",true);
									$('input[name=addrUpdate_HPhoneScd]').val(arr[1]);
									$('input[name=addrUpdate_HPhoneTrd]').val(arr[2]);
								}
							</script>
						</tr>
					</table>
			...
			<div class="modal_footer" style="margin-top: 15px;">
				<div class="modal-footer-btn">
					...
					<button type="button" id="modalSubmit" class="btn"
						data-dismiss="modal" style="color: white;" onclick="addrUpdateformChk()">확인</button>
				</div>
			</div>
		</div>
	</div>
