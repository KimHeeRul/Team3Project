	...
	<div id="my_address">
		...
		<div>
			...
			<div style="display: flex; align-items:center; height:30px;  justify-content: space-between;">
				...
				<form id="addrAddForm" class="MyInfoChkPwForm" onsubmit="return false;">
					<table class="addr_table">
						<tr>
							...
							<td class="addr_input_content">
								...
								<span style="line-height : 28px;">
									<c:out value="${requestScope.MyInfoChkObject.getId()}"/>
								</span>
								</div>
							</td>
						</tr>
						<tr>
							...
							<td class="addr_input_content">
							<input type="password"
								style="width: 25%;" id="name" name="myInfoInput_pwd" maxlength="20"
			...
			<div class="modal_footer" style="margin-top: 15px;">
				<div class="modal-footer-btn">
					...
					<button type="button" id="modalSubmit" class="btn"
						data-dismiss="modal" style="color: white;" onclick="MyInfoPwChk()">확인</button>
				</div>
			</div>
		</div>
	</div>
...
<script type="text/javascript">
	function MyInfoPwChk() {
		const rightPwd = '<c:out value="${requestScope.MyInfoChkObject.getPwd()}"/>';
		const pwd = $("input[name=myInfoInput_pwd]").val();
		
		if(rightPwd === pwd){
			location.href='/goobneMall/service?command=MyInfoUpdateForm';
		}
		else {
			alert("비밀번호를 확인해주세요");
		}
	...
	$(".MyInfoChkPwForm").keypress(function (e) {
        if (e.keyCode === 13) {
        	MyInfoPwChk();
        }
    });
